...
The `home-routes` handler in the `rocks.pho.btc.huobi-transaction-proxy.routes.home` namespace
...
The `home-page` function will in turn call the `rocks.pho.btc.huobi-transaction-proxy.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `rocks.pho.btc.huobi-transaction-proxy.handler` namespace:
...
Request middleware functions are located under the `rocks.pho.btc.huobi-transaction-proxy.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `rocks.pho.btc.huobi-transaction-proxy.dev-middleware` namespace found in
